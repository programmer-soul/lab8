...
public class UDPDatagramClient {
  ...
  public void sendPersons(ResponseManager responsemanager){
    logger.info("Отправка всех объектов клиенту " + addr);
    var data = SerializationUtils.serialize(responsemanager.persons);
    try {
      sendData(data);
      logger.info("Отправлены объекты клиенту " + addr);
    } catch (Exception e) {
      logger.log(Level.SEVERE,"Ошибка отправки объектов.", e);
    }
  }
